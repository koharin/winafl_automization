...
static u32 write_results(void) {
  ...
  if (!strncmp(out_file, "/dev/", 5)) {
    ...
    fd = _open(out_file, O_WRONLY|O_APPEND, DEFAULT_PERMISSION);
...
int get_dir(char *file_path){
  ...
  // remove "\*" from testcase path
  memmove(&file_path[strlen(file_path)-1], &file_path[strlen(file_path)], strlen(file_path)-2);

  while((_findnext(handle, &fd)) == 0){
      strcpy(path, file_path); 
      // ignore non-testcase file
